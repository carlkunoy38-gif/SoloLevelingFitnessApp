      ...
      <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full max-w-lg max-h-[95vh] overflow-y-auto">
        ...
        <div className="flex gap-1.5 p-5 pb-0">
          {STEPS.map((s, i) => (
            <div
              key={s}
              className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${i <= step ? 'bg-blue-600' : 'bg-slate-100'}`}
            />
          ))}
        </div>
        ...
        <div className="p-6">
          ...
          {step === 0 && (
            <div className="space-y-6">
              ...
              <div>
                <div className="text-5xl font-bold text-blue-600 text-center py-3 tabular-nums">
                  {formatKr(bruttoIndkomst)}
                </div>
                <input
                  type="range"
                  min={10000}
          ...
          {step === 1 && (
            <div className="space-y-5">
              ...
              {[
                { emoji: '🏠', label: 'Husleje / boliglån', value: bolig, set: setBolig, max: 25000, step: 500 },
                { emoji: '🛒', label: 'Mad & dagligvarer', value: mad, set: setMad, max: 12000, step: 500 },
                { emoji: '🚗', label: 'Transport', value: transport, set: setTransport, max: 8000, step: 500 },
                { emoji: '💳', label: 'Gæld (lån, afdrag)', value: gæld, set: setGæld, max: 15000, step: 500 },
              ].map(({ emoji, label, value, set, max, step: s }) => (
                <div key={label}>
          ...
          {step === 2 && (
            <div className="space-y-6">
              ...
              <div>
                <div className="text-5xl font-bold text-emerald-600 text-center py-2 tabular-nums">
                  {opsparingPct}%
                </div>
                <div className="text-center text-slate-500 text-sm mb-3">
                  ≈ {formatKr(opsparingBeløb)} kr./mdr. efter skat
                </div>
                <input
                  type="range"
                  min={0}
                  ...
                  step={5}
                  value={opsparingPct}
                  onChange={(e) => setOpsparingPct(Number(e.target.value))}
                  className="w-full accent-emerald-600 h-2"
                />
                <div className="flex justify-between text-xs text-slate-400 mt-1">
                  <span>0%</span>
                  <span className="text-emerald-600 font-medium">↑ Anbefalet: 10–20%</span>
                  <span>50%</span>
                </div>
              </div>
            ...
            <div className="space-y-5 text-center">
              <div className="pt-2">
                ...
                <p className="text-slate-500 mt-2 text-sm leading-relaxed">
                  Vi har sat din profil op. Du kan justere alle detaljer i de enkelte faner.
                </p>
              </div>
        ...
        <div className="px-6 pb-8 pt-2 flex gap-3">
          ...
          >
            {step < 3 ? (
              ...
              <>Vis min økonomi <ArrowRight className="w-4 h-4" /></>
            )}
          </button>
        </div>
      </div>
