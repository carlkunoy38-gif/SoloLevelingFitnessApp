...
export function BudgetBarChart({ analysis }: { analysis: BudgetAnalysis }) {
  const data = analysis.kategoriFordeling
    .filter(k => k.beløb > 0)
    .sort((a, b) => b.beløb - a.beløb)
    .map(k => ({ name: k.kategori.charAt(0).toUpperCase() + k.kategori.slice(1), beløb: Math.round(k.beløb), status: k.status }));
    ...
    <ResponsiveContainer width="100%" height={280}>
      <BarChart data={data} margin={{ top: 10, right: 10, left: 10, bottom: 60 }}>
        ...
        <Bar dataKey="beløb" radius={[6,6,0,0]}>
          {data.map((entry, idx) => <Cell key={idx} fill={STATUS_COLORS[entry.status as keyof typeof STATUS_COLORS]} />)}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  );
